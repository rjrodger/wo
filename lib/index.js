...
exports.register = function (server, pluginOptions, next) {
  ...
  n.on('add',function(meta){
    ...
    var path = meta.route.path
    routeMap[path] = ( routeMap[path] || [] )
    routeMap[path].push( meta )
  })
  ...
  n.on('remove',function(meta){
    ...
    routeMap[path] = ( routeMap[path] || [] )
    for( var i = 0, len = routeMap[path].length; i < len; i++ ) {
      if( routeMap[path].port === meta.port ) {
        routeMap[path].splice(i,1);
        break;
      }
    }
  })
...
internals.handler = function (route, handlerOptions) {
  ...
  var remoteIndex = 0
    ...
    return function (request, reply) {
      ...
      var remotes = routeMap[route.path]
      var remote = remotes[remoteIndex]
      remoteIndex = ( remoteIndex + 1 ) % remotes.length
        ...
        settings.mapUri(request, (err, uri, headers) => {
          ...
          var uri = 'http://127.0.0.1:'+remote.port+request.url.path
          console.log(uri)
