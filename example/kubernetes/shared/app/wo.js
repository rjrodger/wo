    ...
    settings.mapUri(context, request, (err, uri, headers) => {
      ...
      if (settings.passThrough) {
        ...
        if (options.headers.cookie) {
          ...
          const cookieHeader = request.connection.states.passThrough(
...
internals.mapUri = function(protocol, host, port, uri) {
  if (uri) {
    return function(context, request, next) {
      ...
      const address = uri
        .replace(/{protocol}/g, request.connection.info.protocol)
...
internals.agent = function(protocol, settings, connection) {
  ...
  internals.agents[connection.info.uri] =
    internals.agents[connection.info.uri] || {}
  const agents = internals.agents[connection.info.uri]

  const type = protocol === 'http'
    ? 'http'
    : settings.rejectUnauthorized === false ? 'insecure' : 'https'
  if (!agents[type]) {
    agents[type] = type === 'http'
      ? new Http.Agent()
      : type === 'https'
        ? new Https.Agent()
